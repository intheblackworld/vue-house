...
$case_name = $dataList[0]['casename'];
    ...
    $mail->Body = "網站：https://".$case_code.".h35.tw/<BR>姓名：".$name."<BR>預約時間：".$activity."<BR>報名人數：".$count."<BR>電話：".$phone."<BR>城市：".$city.$area."<BR><BR>填表日期：".$datetime."<BR>廣告來源：".$utm_source."<BR>廣告媒介：".$utm_medium."<BR>廣告名稱：".$utm_campaign."<BR>廣告內容：".$utm_content; //設定郵件內容
    ...
    if ($bCheck == true) { //if start
        ...
        try {
            ...
            $url .= "&city=".$city;
            $url .= "&area=".$area;
            $url .= "&message="."預約時間：".$activity."；報名人數：".$count;
